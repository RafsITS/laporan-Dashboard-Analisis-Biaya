...
def create_yearly_trend_chart(summary_df):
    ...
    fig.add_trace(go.Scatter(
        x=years, y=[c/1e6 for c in costs], customdata=costs, mode='lines+markers',
        line=dict(color='#667eea', width=3),
        marker=dict(size=10, color='#764ba2'),
        fill='tozeroy', fillcolor='rgba(102, 126, 234, 0.2)',
        text=[f'Rp {val:,.0f}' for val in costs], textposition='top center',
        hovertemplate='<b>Tahun %{x}</b><br>Total: Rp %{customdata:,.0f}<extra></extra>'
    ))
    fig.update_layout(yaxis=dict(range=[0, max(costs)/1e6 * 1.2], tickformat=",.0f", ticksuffix=" Juta", exponentformat="none"))
    return fig
...
def create_timeline_chart(monthly_df):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=monthly_df['Date'], y=monthly_df['Total Biaya']/1e6, customdata=monthly_df['Total Biaya'],
    ...
    fig.update_yaxes(tickformat=",.0f", ticksuffix=" Juta", exponentformat="none")
    return fig

def create_category_timeline_chart(trend_df):
    if trend_df.empty: return None
    trend_df = trend_df.copy()
    trend_df['Date'] = pd.to_datetime(trend_df['Tahun'].astype(str) + '-' + trend_df['Month_Num'].astype(str) + '-01')
    
    fig = px.line(
        trend_df, x='Date', y=trend_df['Total Biaya']/1e6, color='Keterangan', markers=True,
...
def create_monthly_heatmap(df):
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        text=[[f'{val/1e6:.1f} Juta' if val>0 else '' for val in row] for row in pivot.values],
...
def create_box_plot(df):
    df_box = df.copy()
    df_box['Total Biaya Juta'] = df_box['Total Biaya'] / 1e6
    fig = px.box(df_box, x='Tahun', y='Total Biaya Juta', color='Tahun', color_discrete_sequence=px.colors.qualitative.Set2, title=None)
...
def create_vendor_comparison_chart(vendor_df, top_n=10):
    ...
    fig = go.Figure(data=[go.Bar(
        y=top.index, x=top.values/1e6, customdata=top.values, orientation='h',
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=True, gridcolor='rgba(128, 128, 128, 0.1)', zeroline=False, range=[0, max_val/1e6 * 1.4], tickformat=",.0f", ticksuffix=" Juta", exponentformat="none"),
        margin=dict(r=50)
    )
    return fig
...
def create_scatter_plot(df):
    ...
    stats['Total Biaya Juta'] = stats['Total Biaya'] / 1e6
    fig = px.scatter(
        stats, x='Bulan', y='Total Biaya Juta', hover_data=['Nopol', 'Type', 'Total Biaya'], color='Type',
    ...
    fig.update_traces(hovertemplate='<b>Nopol: %{customdata[0]}</b><br>Tipe: %{customdata[1]}<br>Frekuensi: %{x}x<br>Total: Rp %{customdata[2]:,.0f}<extra></extra>')
    fig.update_yaxes(tickformat=",.0f", ticksuffix=" Juta", exponentformat="none")
    return fig
...
def create_category_chart(category_df):
    ...
    fig = go.Figure(data=[go.Bar(
        y=category_df.index, x=category_df['sum']/1e6, customdata=category_df['sum'], orientation='h',
    ...
    fig.update_layout(
        yaxis=dict(autorange="reversed", automargin=True),
        xaxis=dict(showgrid=True, gridcolor='rgba(128, 128, 128, 0.1)', range=[0, max_val/1e6 * 1.4], tickformat=",.0f", ticksuffix=" Juta", exponentformat="none"),
        margin=dict(r=50)
    )
    return fig
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio("Navigasi", ["Dashboard Utama", "Analisis Detail", "Deteksi Anomali", "Eksplorasi Data", "Laporan Audit"])
    ...
    if page == "Dashboard Utama":
        ...
        metrics = [
            ("💰 Total Pengeluaran", f"{df['Total Biaya'].sum()/1e9:,.1f} Miliar", "Rupiah"),
            ("📈 Rata-rata / Bulan", f"{monthly_avg_cost/1e6:,.1f} Juta", "Pengeluaran per Bulan"),
    ...
    # --- DETEKSI ANOMALI ---
        ...
        with t2:
            if not logic_anom.empty:
                ...
                st.success("Logika kategori konsisten.")
        with t3:
            if not dups.empty:
                ...
                st.success("Data bersih dari duplikasi.")

    # --- EKSPLORASI DATA ---
    elif page == "Eksplorasi Data":
        st.markdown("""
        <div class="dashboard-header" style="padding: 20px; margin-bottom: 20px;">
            <h1 style="font-size: 2em;">🗃️ Eksplorasi Data</h1>
