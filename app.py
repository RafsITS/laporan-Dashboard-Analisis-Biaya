import streamlit as stimport pandas as pdimport plotly.express as pximport plotly.graph_objects as goimport base64import io
...
st.set_page_config(page_title="Analisis Biaya Kendaraan",page_icon="🚗",layout="wide",initial_sidebar_state="expanded")
...
def get_image_as_base64(image_path):"""Convert local image to base64 string"""try:with open(image_path, "rb") as img_file:return base64.b64encode(img_file.read()).decode()except FileNotFoundError:return None
...
def load_custom_css():st.markdown("""<style>/* Import Google Fonts */@import url('https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600;700;800&display=swap');
...
@st.cache_datadef load_and_process_data(file_path=None):try:if file_path is None:file_path = 'Data_Kendaraan_Bersih.csv'
...
def render_chart_card(title, fig, height=450):if fig:fig.update_layout(separators=".,",title_text="",paper_bgcolor='rgba(0,0,0,0)',plot_bgcolor='rgba(0,0,0,0)',font=dict(family='Poppins'),margin=dict(l=40, r=40, t=20, b=40),height=height,xaxis=dict(showgrid=False, zeroline=False),yaxis=dict(showgrid=True, gridcolor='rgba(128, 128, 128, 0.1)', zeroline=False),legend=dict(font=dict(family='Poppins')))st.markdown(f"""<div class="chart-container"><div style="font-weight: 700; font-size: 1.1em; margin-bottom: 15px; text-transform: uppercase; border-bottom: 2px solid rgba(128,128,128,0.1); padding-bottom: 10px;">{title}</div>""", unsafe_allow_html=True)st.plotly_chart(fig, use_container_width=True)st.markdown("</div>", unsafe_allow_html=True)
...
def calculate_yearly_summary(df):summary = df.groupby('Tahun').agg({'Total Biaya': ['sum', 'mean', 'count']}).round(0)summary.columns = ['Total_Pengeluaran', 'Rata_Rata', 'Jumlah_Transaksi']return summary

def get_top_vendors(df, top_n=10):return df.groupby('Vendor_Clean')['Total Biaya'].sum().sort_values(ascending=False).head(top_n)

def get_top_units(df, top_n=10):top_units = df.groupby(['Nopol', 'Type']).agg({'Total Biaya': 'sum', 'Bulan': 'count'})top_units.columns = ['Total_Biaya', 'Frekuensi_Servis']return top_units.sort_values(by='Total_Biaya', ascending=False).head(top_n)

def calculate_monthly_category_trend(df):trend = df.groupby(['Tahun', 'Month_Num', 'Bulan', 'Keterangan'])['Total Biaya'].sum().reset_index()
...
def calculate_monthly_trend(df):monthly = df.groupby(['Tahun', 'Month_Num', 'Bulan'])['Total Biaya'].sum().reset_index()
...
def calculate_category_distribution(df):return df.groupby('Keterangan')['Total Biaya'].agg(['sum', 'count']).sort_values('sum', ascending=False)

def calculate_type_statistics(df):type_stats = df.groupby('Type').agg({'Total Biaya': ['sum', 'mean', 'count'], 'Nopol': 'nunique'})type_stats.columns = ['Total_Biaya', 'Avg_Biaya', 'Transaksi', 'Jumlah_Unit']return type_stats.sort_values('Total_Biaya', ascending=False)

def detect_duplicates(df):return df[df.duplicated(subset=['Bulan', 'Tahun', 'Nopol', 'Total Biaya', 'Vendor_Clean'], keep=False)]

--- Chart Creators ---

def format_currency_text(val):if val >= 1e9:return f"Rp {val/1e9:,.1f} Miliar"return f"Rp {val/1e6:,.0f} Juta"

def generate_tick_labels(max_value, num_ticks=6):'''Generates specific tick values and texts to avoid 1000 Juta and B/Billion'''if max_value <= 0:return [0], ["0 Juta"]
...
def create_yearly_trend_chart(summary_df):if summary_df.empty: return Noneyears = summary_df.index.tolist()costs = summary_df['Total_Pengeluaran'].tolist()
...
def create_vendor_pie_chart(vendor_df):if vendor_df.empty: return Nonelabels = vendor_df.index[:8].tolist() + [f'Others ({len(vendor_df) - 8} Vendor)'] if len(vendor_df) > 8 else vendor_df.index.tolist()values = vendor_df.values[:8].tolist() + [vendor_df.values[8:].sum()] if len(vendor_df) > 8 else vendor_df.values.tolist()fig = go.Figure(data=[go.Pie(labels=labels, values=values, hole=0.4,marker=dict(colors=px.colors.qualitative.Set3, line=dict(color='white', width=2)),textinfo='percent', hovertemplate='<b>%{label}</b><br>Rp %{value:,.0f}<extra></extra>')])return fig

def create_timeline_chart(monthly_df):if monthly_df.empty: return Nonemonthly_df = monthly_df.copy()monthly_df['Date'] = pd.to_datetime(monthly_df['Tahun'].astype(str) + '-' + monthly_df['Month_Num'].astype(str) + '-01')
...
def create_category_timeline_chart(trend_df, top_n=5):if trend_df.empty: return Nonetrend_df = trend_df.copy()
...
def create_monthly_heatmap(df):pivot = df.pivot_table(values='Total Biaya', index='Bulan', columns='Tahun', aggfunc='sum', fill_value=0)order = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember']pivot = pivot.reindex(order, axis=0, fill_value=0)
...
def create_box_plot(df):fig = px.box(df, x='Tahun', y='Total Biaya', color_discrete_sequence=['#667eea'], title=None)fig.update_layout(showlegend=False)
...
def create_vendor_comparison_chart(vendor_df, top_n=10):top = vendor_df.head(top_n)
...
def create_scatter_plot(df):stats = df.groupby('Nopol').agg({'Total Biaya': 'sum', 'Bulan': 'count', 'Type': 'first'}).reset_index()stats['Avg Biaya'] = stats['Total Biaya'] / stats['Bulan']
...
def create_type_distribution_chart(type_stats):if type_stats.empty: return None
...
def create_category_chart(category_df):fig = go.Figure(data=[go.Bar(y=category_df.index, x=category_df['sum'], customdata=category_df['sum'], orientation='h',marker=dict(color=category_df['sum'], colorscale='Viridis', showscale=False),text=[format_currency_text(val) for val in category_df['sum']],textposition='outside', cliponaxis=False,hovertemplate='<b>%{y}</b><br>Rp %{customdata:,.0f}<extra></extra>')])
...
def render_footer():sby_logo = get_image_as_base64('SBY.png') or ''its_logo = get_image_as_base64('ITS.png') or ''
